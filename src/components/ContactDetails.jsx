         ...
         <div className='space-y-4'>
            <div className='flex items-center justify-between'>
               <div className='flex items-center'>
                  <div
                     className={`w-10 h-10 rounded-full   ${getColorClass(
                        appTheme.colorTheme,
                        'buttonbg'
                     )} flex items-center justify-center mr-2 md:mr-4`}
                  >
                     <svg
               ...
               <a
                  ...
                  target='_blank'
                  className={`text-gray-900 dark:text-gray-300 rounded-md w-24 flex items-center justify-center cursor-pointer   transition-colors duration-300  ${getColorClass(
                     appTheme.colorTheme,
                     'shadow'
                  )}
                  ${getColorClass(
                     appTheme.colorTheme,
                     'background'
                  )} hover:opacity-80 hover:scale-105 active:scale-95 py-1 px-2 space-grotesk text-sm border border-solid border-gray-900 dark:border-gray-300`}
               >
                  Portfolio
                  <MdArrowOutward className='ml-1 size-5' />
               </a>
            </div>
            <div className='flex items-center '>
               <div
               ...
               <a
                  ...
                  className={`rounded-md flex items-center justify-center cursor-pointer transition-colors duration-300 ${getColorClass(
                     appTheme.colorTheme,
                     'buttonbg'
                  )} ${getColorClass(
                     appTheme.colorTheme,
                     'shadow'
                  )} hover:opacity-80 hover:scale-105 active:scale-95 p-1 ml-3 text-gray-900 dark:text-gray-300`}
               >
         ...
         <div className='flex justify-center space-x-6 mt-8'>
            {socials.map((social, index) => (
               <a
                  ...
                  href={social.url}
                  target='_blank'
                  className={`size-14 rounded-full flex items-center justify-center cursor-pointer   transition-colors duration-300 ${getColorClass(
                     appTheme.colorTheme,
                     'buttonbg'
                  )} ${getColorClass(
                     appTheme.colorTheme,
